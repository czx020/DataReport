...
ggplot(mydata,aes(x = color,y = depth ,fill=cut))+
  geom_bar(position = 'dodge',colour = 'transparent',stat = 'identity') +
  facet_grid(.~cut)+#???#先不分面，观察图形，再按cut值不同按行分面，观察图形
  scale_fill_wsj() +
...
ggplot(mydata,aes(x = color,y = depth ,fill=cut))+
  geom_bar(position ='stack',colour ='transparent',stat ='identity')+coord_polar(theta = "x",start=0)+scale_fill_wsj()  
#先不转换坐标系，观察图形，Y轴退化为半径，X轴退化为圆周
#说明：scale_fill_wsj() 设置序列的填充颜色,有5种参数


#堆积柱形图——沿着y轴旋转
ggplot(mydata,aes(x = color,y = depth ,fill=cut))+
  geom_bar(position = 'stack',colour = 'transparent',stat = 'identity')+
  coord_polar(theta = "x",start=0)+#先不转换坐标系，观察图形，X轴退化为半径，y轴退化为圆周
  scale_fill_wsj()
...
ggplot(mydata2,aes(x = cut,y = depth,fill = color))+
  geom_bar(width=1,stat = 'identity', position = "dodge") + #柱形图为簇状，position为dodge
  scale_fill_wsj() +
  geom_text(aes(label = depth ),position = position_dodge(0.5),hjust = .5,vjust =.5)#标签层的position必须也是position_dodge(width = 1) 


ggplot(mydata2,aes(x = cut,y = depth,fill = color))+
  geom_bar(width=1,stat = 'identity', position = 'dodge') +
  geom_text(aes(label = depth ),position = position_dodge(width = 1),hjust = .5,vjust =.5) +
  facet_grid(.~color)+#??? +  #将所有柱子按颜色按行分面
  scale_fill_wsj()
...
ggplot(mydata2,aes(x = cut,y = depth,fill = color))+
  geom_bar(width=1,stat = 'identity') +
  scale_fill_wsj() +
  geom_text(aes(label = depth ),position = position_stack(.5),hjust = .5,vjust =.5)#多序列堆积柱形图添加标签 
...
ggplot(mydata2,aes(x = cut,y = depth,fill = color))+
  ...
  geom_text(aes(label = depth ),position = position_stack(vjust = .5),hjust = .5,vjust =.5) +
  facet_grid(.~color) +
  coord_polar()+#将分面后的柱形图转换成极坐标，coord_polar默认设置，观察旋转是按哪个轴为中心
  scale_fill_wsj()
