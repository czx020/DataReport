...
ggplot(mydata)+
  geom_??? (aes(??? , fill = class)) + #绘制自由矩形图，xmin与xmax分别为xstart与xend,y轴设置类似
  scale_fill_wsj()
...
ggplot(mydata) +
  geom_segment(
    ...
    arrow = ??? (length = unit(0.5,"cm")),#添加箭头指示
    size = 1.5
  ) +
  scale_colour_wsj() 
...
ggplot(mydata) +
  geom_??? (aes(??? , colour = class) , size = 1.5) +#绘制线范围图x值设置为xstart,y的范围为[ymin,ymax]
  scale_colour_wsj() 
...
#
ggplot(mydata) +
  geom_???(aes(x = long , y = lat , group  = group , fill = class),colour = "white") +#绘制多边形图，x为long , y 为lat , group为group , fill为class
  scale_fill_wsj()

#按照X轴圆周化：
ggplot(mydata) +
  geom_???(aes(x = long , y = lat , group  = group , fill = class),colour = "white") +
  coord_polar(theta = 'x') +
  scale_x_continuous(expand = c(0,0)) +
  scale_fill_wsj()
